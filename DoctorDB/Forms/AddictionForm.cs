...
{
    public partial class AddictionForm : Form
    {
        ...
        private void GetData()
        {
            using (SqlConnection conn = new SqlConnection(GlobalConfig.ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("SELECT * FROM AddictionPatient", conn))
                {
                    ...
                    using (SqlDataReader sdr = cmd.ExecuteReader())
                    {
                        try
                        {
                            while (sdr.Read())
                            {
                                ...
                                AddictionPatientLb.Items.Add(FullName);
                            }

                            // Sorting the Listbox in Ascending order
                            AddictionPatientLb.Sorted = true;
                        }
                        catch (Exception ex)
                        {
        ...
        private void ChangePatientLabels(String IDNUmber)
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("GetSpecificAddictionPatient", conn))
                {
        ...
        private void LoadRecords(string idNumber)
        {
            ...
            {
                conn.Open();
                using (SqlCommand cmd = new SqlCommand("GetAddictionPatientRecords", conn))
                {
                    cmd.CommandType = CommandType.StoredProcedure;
                    cmd.Parameters.AddWithValue("@IDNumber", idNumber);
        ...
        {
            if (AddictionPatientLb.SelectedIndex > -1)
            {
                string[] splitName = AddictionPatientLb.Text.Split(' ');
                string query = "";

                // This query that will get the ID number of the Patient that is selected in the listBox
                if (splitName.Length == 2)
                {
                    query = "SELECT * FROM AddictionPatient WHERE FirstName = '" + splitName[0] + "' AND LastName = '" + splitName[1] + "'";
                }
                else if (splitName.Length == 3)
                {
                    query = "SELECT * FROM AddictionPatient WHERE FirstName = '" + splitName[0] + "' AND LastName = '" + splitName[1] + "' AND MiddleName = '" + splitName[2] + "';";
                }
        ...
        private void txtSearchBox_TextChanged(object sender, EventArgs e)
        {
            ...
            foreach (string str in nameList)
            {
                ...
                {
                    AddictionPatientLb.Items.Add(str);
                }
            }
        }
        ...
        private void LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            var dForm = Application.OpenForms.OfType<DashboardForm>().Single();
            dForm.ManageAddictionPatientMethodInDashboard(idNo, FullName);
        }
        ...
        private void recordsDatagrid_SelectionChanged(object sender, EventArgs e)
        {
            ...
            //// This changes as you click different rows - Need to only use the top Row
            lastVisit = recordsDatagrid.CurrentRow.Cells[3].Value.ToString();
            lastVisitlbl.Text = lastVisit;
        }
    }
}
