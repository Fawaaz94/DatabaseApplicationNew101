...
{
    public partial class ChronicPatients : Form
    {
        // This is a Data Table created which will hold all the Patients and their data
        private DataTable dtFullNames;
        ...
        {
            //PatientLB.DataSource = GetData();
            //PatientLB.DisplayMember = "Full_Name";
            GetData();
        ...
        private /*DataTable*/ void GetData()
        {
            // Creating the instance of the Datatable
            //dtFullNames = new DataTable();

            // Adding a column named "Full_Name" to the Datatable
            //dtFullNames.Columns.Add("Full_Name", typeof(string));
            ...
            using (SqlConnection conn = new SqlConnection(GlobalConfig.ConnectionString))
            {
                ...
                using (SqlCommand cmd = new SqlCommand("GetPatients", conn))
                {
            ...
            // Returns the Datatable that we populated
            //return dtFullNames;
        }
        ...
        private void ChangePatientLabels(String IDNUmber)
        {
            ...
            SqlCommand cmd = new SqlCommand("GetSpecificChronicPatient", GlobalConfig.SQLCONN);
            cmd.CommandType = CommandType.StoredProcedure;
        ...
        private void PatientLB_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (PatientLB.SelectedIndex > -1)
            {
                ...
                string query = "SELECT * FROM Patient WHERE FirstName = '" + splitName[0] + "' AND LastName = '" + splitName[1] + "'"; ;
                //MessageBox.Show(string.Format("{0} {1}", splitName[0], splitName[1]));

                //// This query that will get the ID number of the Patient that is selected in the listBox
                //if (splitName.Length == 2)
                //{
                //    query = "SELECT * FROM Patient WHERE FirstName = '" + splitName[0] + "' AND LastName = '" + splitName[1] + "'";
                //}
                //else if (splitName.Length == 3)
                //{
                //    query = "SELECT * FROM Patient WHERE FirstName = '" + splitName[0] + "' AND LastName = '" + splitName[1] + "' AND MiddleName = '" + splitName[2] + "';";
                //}
        ...
        {
            //DataView dvFullName = dtFullNames.DefaultView;

            //dvFullName.RowFilter = "Full_Name LIKE '%" + txtSearchBox.Text + "%'";
            //PatientLB.Visible = true;
            PatientLB.Items.Clear();
        ...
        private void FirstNameLbl_LinkClicked(object sender, LinkLabelLinkClickedEventArgs e)
        {
            ...
            var dForm = Application.OpenForms.OfType<DashboardForm>().Single();
            dForm.ManagePatientMethodInDashboard(idNo, FullName);
        }

        //
        private void recordsDatagrid_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            var medicationDisplayForm = new MedicationDisplay(idNo);

            medicationDisplayForm.Show();

            medicationDisplayForm.FormClosing += (s, EventArgs) => this.Show();
        }
    }
}
